...
- [x] 게임 시작 문구를 출력한다.
- [x] 다리의 길이를 입력받는다.
- [x] 입력받은 다리 길이로 다리를 만든다.
- [x] 시도 회수를 구한다.
- [x] 이동할 칸을 입력받는다.
- [x] 이동시 다리 건너기 결과를 출력한다.
- [x] 갈 수 없는 곳이라면 재시도 또는 종료 여부를 입력받는다.
- [x] 갈 수 있는 곳이라면 다리를 모두 건널때까지 계속 반복한다.
- [x] 게임을 종료했을 경우 최종 결과를 출력하고 종료한다.
- [x] 다리를 모두 건넜을 경우 최종 결과를 출력하고 종료한다.
